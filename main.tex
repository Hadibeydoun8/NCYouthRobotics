...
static int16_t apply_dead_zone(int16_t value, int16_t deadzone=7000) {
    ...
    // Multiply by -1 to invert Y-axis: controller reports up as negative,
    // but robot expects positive for forward movement
    return sign * static_cast<int16_t>(scaled * max_val) * -1;
}
\end{lstlisting}

\textbf{Why needed?} Without dead zone, small stick drift would cause unintended robot movement. The final multiplication by -1 inverts the Y-axis because game controllers typically report upward stick movement as negative values, but robots expect positive values for forward motion.

\textbf{Note}: This function is used for both left and right sticks. In a more generic implementation, the Y-axis inversion could be handled outside the dead zone function to make it reusable for X-axis processing as well.
...
\textbf{Performance Note}: The rate object is created inside the loop. For optimal performance in production code, consider creating it once outside the loop. However, this implementation is sufficient for a 5 Hz control rate.
...
\textbf{Performance Note}: The \texttt{delay(100)} blocks the entire Arduino loop. For applications requiring multiple concurrent tasks (e.g., sensor reading, LED indicators), consider using \texttt{millis()} for non-blocking timing. However, for this simple motor control application, the blocking delay is adequate.
...
\begin{lstlisting}[style=bashstyle]
# Set ROS_DOMAIN_ID on both machines (must be the same)
export ROS_DOMAIN_ID=0

# ROS2 will automatically discover nodes on the same network
# No need to set ROS_IP (ROS2 uses DDS discovery)
